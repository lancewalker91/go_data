...
//绘制关闭按钮
#define SCENE_ITEM_BORDER        (1<<0)
//绘制全屏按钮
#define SCENE_ITEM_FULLSCREEN_BNT        (1<<1)
//Item是否禁止在场景中移动
#define SCENE_ITEM_FORBIDDEN_MOVE        (1<<2)
//Item是否禁止在场景中移动
#define SCENE_ITEM_ONLY_PREVIEW       (1<<3)
...
EXPORT int get_video_real_render_rate();
...
/** 获取缓存中渲染的文理*/
EXPORT gs_texture_t* obs_current_render_tex();
...
/**
*直接调用插件的get_properties接口，返回没有与source setting合并的obs_properties_t
*obs_output_properties 返回的是与source setting合并了的
*用obs_properties_destroy释放
*/
EXPORT obs_properties_t *obs_source_default_properties(const obs_source_t *source);

/**
*直接修改对应的property_t
*/
EXPORT void obs_source_property_modified(const obs_source_t *source, const char *name);

EXPORT property_t *obs_source_get_property(const obs_source_t *source, const char* name);

/** Updates settings for this source */
EXPORT void obs_source_update(obs_source_t *source, obs_data_t *settings);

EXPORT void enter_preview_render();
EXPORT void leave_preview_render();
...
/** Gets the settings string for a source add ref */
EXPORT obs_data_t *obs_source_get_settings(const obs_source_t *source);

/** Gets the settings string for a source but not add ref*/
EXPORT obs_data_t *obs_source_get_settings_not_addref(const obs_source_t *source);
...
/** 获取async_frames缓存的大小 */
EXPORT int obs_source_async_frames_count(obs_source_t *source);
...
/** Gets the base height for a source (not taking in to account filtering) */
EXPORT void obs_source_set_parameter(obs_source_t *source, void *param);
...
/** Determines whether a source is within a scene */
EXPORT bool source_in_scene_item(obs_scene_t *scene, const obs_source_t *source);
...
EXPORT void obs_scene_lock(obs_scene_t *scene);
EXPORT void obs_scene_unlock(obs_scene_t *scene);
...
EXPORT obs_sceneitem_t *obs_scene_add_without_source(obs_scene_t *scene);

EXPORT void add_tick_source(struct obs_scene *scene, struct obs_source * source);

EXPORT void remove_tick_source(struct obs_scene *scene, struct obs_source * source);

/** 关联item与source */
EXPORT void bind_item_source(obs_sceneitem_t* item ,obs_source_t *source);
/** 解除item与source的关联 */
EXPORT void unbind_item_source(obs_sceneitem_t* item);
/* */
EXPORT uint32_t item_get_window_flags(obs_sceneitem_t* item);
/* */
EXPORT void item_set_window_flags(obs_sceneitem_t* item, uint32_t flags);
/* */
EXPORT void item_set_window_flag(obs_sceneitem_t* item, uint32_t flag);
/* */
EXPORT void item_clear_window_flag(obs_sceneitem_t* item, uint32_t flag);
/* */
EXPORT bool item_has_window_flag(obs_sceneitem_t* item, uint32_t flag);

EXPORT void item_closebtn_box_transfom(struct matrix4 *transform, const obs_sceneitem_t *item, uint32_t width, uint32_t height);

//item 的默认文理是否显示
EXPORT void item_image_visible(obs_sceneitem_t* item, bool b);
EXPORT void set_image_tex(obs_sceneitem_t* item, gs_texture_t *tex);
...
//设置绘制背景
EXPORT void item_draw_background(obs_sceneitem_t* item, bool b);
...
/** Removes a scene item. */
EXPORT void obs_scene_add_item(obs_scene_t *scene, obs_sceneitem_t *item);
...
//
//EXPORT bool obs_sceneitem_scale_by_source_size(const obs_sceneitem_t *item);
//EXPORT void obs_sceneitem_set_scale_by_source_size(obs_sceneitem_t *item, bool s);

EXPORT bool obs_sceneitem_can_strech(const obs_sceneitem_t *item);
EXPORT void obs_sceneitem_set_can_strech(obs_sceneitem_t *item, bool strech);

/**get the bottom item*/
EXPORT bool obs_sceneitem_bottom(obs_scene_t *scene, obs_sceneitem_t *item);
/**get the pre item*/
EXPORT obs_sceneitem_t *obs_sceneitem_pre(obs_sceneitem_t *item);

EXPORT obs_sceneitem_t *obs_sceneitem_next(obs_sceneitem_t *item);
/* ------------------------------------------------------------------------- */
/* Outputs */
...
EXPORT int obs_output_get_defer_time(const obs_output_t *output);
...
EXPORT void initialize_win32_crash_handler(void);

typedef void(*pstack_callback)(const char *, void *param);
EXPORT void pstack(unsigned long pid, pstack_callback callback, void *param);
