...
inline uint32_t get_audio_channels(enum speaker_layout speakers)
{
	switch (speakers) {
...
EXPORT void audio_output_volmeter_open(audio_t *audio);
EXPORT void audio_output_volmeter_close(audio_t *audio);
EXPORT void connect_volmeter_levels_updated(audio_t *audio, signal_callback_t callback, void *data);
EXPORT void disconnect_volmeter_levels_updated(audio_t *audio, signal_callback_t callback, void *data);

//gn---
EXPORT inline enum AVSampleFormat convert_audio_format(enum audio_format format);
EXPORT inline  uint64_t convert_speaker_layout(enum speaker_layout layout);  //obs layoutתffmpeg layout
#ifdef __cplusplus
}
#endif
